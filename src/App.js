...
function App() {
  ...
  const quantityChange = (item) => {
    ...
    const newCART = CART.map((each)=>{
      return each.id===item.id ? {...item,amount:item.amount +1 }: item
    })
    setCART(newCART)
  } 
  const quantityDecrease = (item) => {
    const newCART = CART.map((each)=>{
      return each.id===item.id ? {...item,amount:item.amount>0 ? item.amount-1 : 0} : item
    })
    setCART(newCART)
  }
  return (
    <div className="App">
       <ShoppingHeader count={CART.length} setShow={setShow}/>
      {show ? <ShoppingList data={data} addToCart={addToCart}/> : <><Cart CART={CART} quantityChange={quantityChange} quantityDecrease={quantityDecrease}/>
